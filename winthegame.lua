...
function WinScreen:new()
    Winfont = love.graphics.newFont("assets/Mojangles.ttf", 20)
    self.x = math.random(800*ScaleX-80)
    self.y = math.random(600*ScaleY-180)
...
function WinScreen:draw()
    if self.dx == 0 or self.dy == 0 then
        ...
        if self.x >= love.graphics.getWidth()-80 or self.x < 0 then
            self.dx = - self.dx
        end
